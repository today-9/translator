...
「見出し<TAB>意味」形式で書くと自分の訳が先頭に表示され、EJDict の訳も
続けて併記される。保存すれば即反映。古い辞書ゆえの珍訳
(例: PC = Peace Corps 平和部隊)には、ここで現代の訳を足すのが早い。

## 使い方

```powershell
uv run translator-startup run   # 常駐開始(コンソールなし。ターミナルは閉じてよい)
```

`uv run translator` でも起動できるが、こちらはターミナルに紐づくため
閉じるとアプリも終了する(ログを見たいとき・デバッグ用)。

- テキストを選択して **`Ctrl+C` を2連打**(config.tomlで通常の組み合わせにも変更可)
- **`Ctrl+Alt+T`** で入力窓を開き、打ち込んだ単語・文章を翻訳(Enter=翻訳 / Esc=閉じる)。
  トレイメニューの「入力して翻訳...」からも開ける
- ポップアップは Esc か画面クリックで消す(放置時は5分で自動クローズ)
...
wscript.exe + VBScript 経由でコンソールなし起動するショートカットを
`shell:startup` に作る。ターミナルから `uv run translator` で起動した場合と
違い、ターミナルを閉じてもアプリは終了しない。

実装メモ: pythonw.exe は使っていない。uv 配布の Python は未署名のため、
Smart App Control 等のアプリ制御が実行実績の少ない pythonw.exe を
評判ベースでブロックすることがある(WinError 4551)。python.exe を
ウィンドウ非表示で起動する方式(`run` は CREATE_NO_WINDOW、ログイン起動は
署名済み wscript.exe 経由)ならブロックされない。多重起動はミューテックスで
ガードしているので、二重に実行しても2つ常駐することはない。
...
コードの構造・カスタマイズ方法・実装上の注意点は **[SPEC.md](SPEC.md)** にまとめてある。
...
```powershell
git clone https://github.com/today-9/translator.git ; cd translator
uv sync --all-extras
uv run translator-setup company   # 辞書 + FuguMT + Qwen のみ(PLaMoを落とさない)
uv run translator-startup run     # コンソールなしで常駐開始
```

ログイン時の自動起動まで許可が取れているなら `uv run translator-startup`
(登録)でもよい。
...
- **フックなしモード(EDR対策)**: `translator-setup company` は `config.toml` を
  `[hotkey] mode = "none"` で生成する。キーボード/マウスの低レベルフックを一切
  張らず、OS公認の `RegisterHotKey` で待ち受けるため、CrowdStrike 等の EDR に
  キーロガーとして検知されない(詳細は [SPEC.md](SPEC.md) §8)
- 未インストールのエンジンはトレイメニューに表示されず、誤って選ぶ余地はない
- `config.toml` が PLaMo を指していても起動時に自動で FuguMT へフォールバック
- 翻訳処理は完全ローカル。社内文書がネットに出ることはない

### none モードの使い方(global との違い)

| | global(既定) | none(会社向け) |
|---|---|---|
| 選択翻訳 | 選択して Ctrl+C 2連打 | **コピーしてから** `Ctrl+Alt+C` |
| 入力窓 | `Ctrl+Alt+T` | `Ctrl+Alt+T` / トレイ左クリック |
| ポップアップを閉じる | Esc / クリック / 5分 | Esc / 他所クリック / 5分 |
| キーフック | あり(EDR検知有) | **なし**(EDR安全) |

### 導入前の注意

none モードでも常駐アプリがホットキー登録とクリップボード読み取りを行う。
EDR に検知される可能性は大きく下がるが、**私物ツールの持ち込み可否は組織の
ポリシー次第**。隠れて使わず事前に情シスへ申請するのが安全
(全コードが読める・外部通信ゼロ・モデルもローカル、が説明材料になる)。
検知回避のための難読化やフック隠蔽は絶対にしないこと(良性ツールが
マルウェア扱いに格上げされる)。
